...
(function () {
  ...
  function flush() {
    ...
    var batch = pending;
    pending = [];
    // Deliberately NOT disconnecting. disconnect() DISCARDS queued records, and
    // this tool renders in bursts -- the build banner and all 46 table captions
    // were silently dropped that way, never even reaching the missing-strings
    // report. Staying connected is safe: re-visiting a node we already
    // translated resolves to one of our own outputs and is a no-op, so there is
  ...
  var observer = new MutationObserver(function (records) {
    ...
    if (pending.length && !scheduled) {
      ...
      // and never even reached the missing-strings report. setTimeout still
      // fires (throttled to ~1s in background tabs, which is fine here).
      setTimeout(flush, 0);
    }
  });
