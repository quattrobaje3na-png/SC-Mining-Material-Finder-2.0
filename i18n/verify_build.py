"""Correctness gate for the Mining Material Finder. Runs after build_dicts.py
and BEFORE anything publishes.
...
def check_markup(src: Path, out_dir: Path, lang: str, f: Failures) -> None:
    ...
    # The mining tool ships TWO data files; both must be byte-identical.
    for name in ("mining_compiled.json", "mining_equipment.json"):
        src_data, out_data = src / name, out_dir / name
        if not src_data.exists():
            f.add(f"[{lang}] source data file missing: {name}")
        elif not out_data.exists():
            f.add(f"[{lang}] {name} missing from output")
        elif src_data.read_bytes() != out_data.read_bytes():
            f.add(f"[{lang}] {name} was modified; it must be copied verbatim")
